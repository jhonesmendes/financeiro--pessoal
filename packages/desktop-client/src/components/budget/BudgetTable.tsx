...
export function BudgetTable(props: BudgetTableProps) {
  ...
  const onShowNewGroup = () => setIsAddingGroup(true);
  const onHideNewGroup = () => setIsAddingGroup(false);
    ...
    >
      ...
      >
        <Text style={{ fontSize: 13, fontWeight: 600 }}>
          {`${t('Categories')} — ${monthLabel}`}
        </Text>
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <Button variant="normal" onPress={onShowNewGroup}>
            {`+ ${t('Add group')}`}
          </Button>
          <Button variant="primary" onPress={() => navigate('/accounts', { state: { addTransaction: true } })}>
            {`+ ${t('New Transaction')}`}
          </Button>
        </View>
      </View>
