...
import * as whatsappApp from './app-whatsapp/app-whatsapp.js';
...
app.use('/whatsapp', whatsappApp.handlers);
...
export async function run() {
  ...
  // Reconnect WhatsApp on boot if it was previously paired (auth session
  // saved on disk). Pairing/group selection works independently of the
  // budget-writing config below.
  const whatsappConnection = await import('./app-whatsapp/whatsapp-connection.js');
  void whatsappConnection.connect();
