...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      ...
      // @actual-app/api ships a large bundle that mixes CJS (__dirname) and
      // ESM (top-level await) idioms internally; inlining it here confuses
      // Node's module-format detection. Keep it external and let Node
      // resolve it from node_modules at runtime instead (same as express,
      // better-sqlite3, etc.).
