...
function getTagNotes(
  ...
  if (
    (categoryName === 'Entretenimento' || categoryName === 'Vestuário') &&
...
function getChildTagNotes(categoryName: string, amount: number): string {
  ...
  if (categoryName === 'Alimentação') tags.push('#groceries');
  else if (categoryName === 'Restaurantes') tags.push('#dining-out');
  else if (categoryName === 'Entretenimento') tags.push('#entertainment');
  else if (categoryName === 'Vestuário') tags.push('#clothing');
  else if (categoryName === 'Saúde') tags.push('#medical');
  else if (categoryName === 'Presentes') tags.push('#gift');
...
function getStartingBalanceCat(categories: CategoryGroupEntity[]) {
  return categories.find(c => c.name === 'Saldos Iniciais').id;
}
...
function extractCommonThings(
  ...
  return {
    ...
    billCategories: groups.find(g => g.name === 'Contas').categories,
...
async function fillPrimaryChecking(
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    if (payee.name === 'Deposit') {
      category = incomeGroup.categories.find(c => c.name === 'Renda');
    } else {
    ...
    if (random() < 0.2) {
      ...
      const pickCat = () =>
        payee === incomePayee
          ? incomeGroup.categories.find(c => c.name === 'Renda')
  ...
  for (const month of months) {
    ...
    if (monthUtils.isBefore(date, currentDay)) {
      transactions.push({
        ...
        account: account.id,
        date,
        category: billCategories.find(c => c.name === 'Energia').id,
        notes: '#utilities',
      });
    }
    ...
    if (monthUtils.isBefore(date, currentDay)) {
      transactions.push({
        ...
        account: account.id,
        date,
        category: billCategories.find(c => c.name === 'Água').id,
        notes: '#utilities',
      });
    }
    ...
    if (monthUtils.isBefore(date, currentDay)) {
      transactions.push({
        ...
        account: account.id,
        date,
        category: billCategories.find(c => c.name === 'Financiamento').id,
        notes: '#mortgage',
      });
    }
    ...
    if (monthUtils.isBefore(date, currentDay)) {
      transactions.push({
        ...
        account: account.id,
        date,
        category: billCategories.find(c => c.name === 'Celular').id,
        notes: '#subscription #utilities',
      });
    }
  }
...
async function fillInvestment(handlers, account, payees, groups) {
  ...
  for (let i = 0; i < numTransactions; i++) {
    ...
    const category = incomeGroup.categories.find(c => c.name === 'Renda');
...
async function createBudget(accounts, payees, groups) {
    ...
    batchMessages(async () => {
      for (const month of months) {
        if (
          ...
          void setBudget(month, category('Alimentação'), 40000);
          void setBudget(month, category('Restaurantes'), 30000);
          void setBudget(month, category('Entretenimento'), 10000);
          void setBudget(month, category('Vestuário'), 3000);
          void setBudget(month, category('Geral'), 50000);
          void setBudget(month, category('Presentes'), 7500);
          void setBudget(month, category('Saúde'), 10000);

          void setBudget(month, category('Celular'), 7500);
          void setBudget(month, category('Internet'), 6000);
          void setBudget(month, category('Financiamento'), 120000);
          void setBudget(month, category('Água'), 9000);
          void setBudget(month, category('Energia'), 10000);
        } else {
          setBudgetIfSpent(month, category('Alimentação'));
          setBudgetIfSpent(month, category('Restaurantes'));
          setBudgetIfSpent(month, category('Entretenimento'));
          setBudgetIfSpent(month, category('Vestuário'));
          setBudgetIfSpent(month, category('Geral'));
          setBudgetIfSpent(month, category('Presentes'));
          setBudgetIfSpent(month, category('Saúde'));

          setBudgetIfSpent(month, category('Celular'));
          setBudgetIfSpent(month, category('Internet'));
          setBudgetIfSpent(month, category('Financiamento'));
          setBudgetIfSpent(month, category('Água'));
          setBudgetIfSpent(month, category('Energia'));
        }
      }
    }),
  );
      ...
      for (const month of months) {
        if (
          ...
          if (available - 403000 > 0) {
            void setBudget(month, category('Poupança'), available - 403000);
  ...
  // setBudget(monthUtils.currentMonth(), category('Poupança'), toBudget);
...
export async function createTestBudget(handlers: Handlers) {
  ...
  const newCategoryGroups: Array<CategoryGroupDefinition> = [
    ...
    },
    {
      ...
      is_income: true,
      categories: [
        { name: 'Renda', is_income: true },
        { name: 'Diversos', is_income: true },
        { name: 'Saldos Iniciais', is_income: true },
      ],
    },
  ];
