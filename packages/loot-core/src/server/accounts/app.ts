...
export type AccountHandlers = {
  ...
  'whatsapp-status': typeof whatsappStatus;
  'whatsapp-connect': typeof whatsappConnect;
  'whatsapp-save-config': typeof whatsappSaveConfig;
  'whatsapp-select-group': typeof whatsappSelectGroup;
  'whatsapp-refresh-groups': typeof whatsappRefreshGroups;
  'whatsapp-logout': typeof whatsappLogout;
...
async function whatsappRequest(path: string, body: unknown = {}) {
  const userToken = await asyncStorage.getItem('user-token');
  if (!userToken) {
    return { error: 'unauthorized' };
  }
  ...
  return post(serverConfig.WHATSAPP_SERVER + path, body, {
    'X-ACTUAL-TOKEN': userToken,
  });
}

async function whatsappStatus() {
  return whatsappRequest('/status');
}

async function whatsappConnect() {
  return whatsappRequest('/connect');
}

async function whatsappSaveConfig(config: {
  accountId: string;
  syncId: string;
  ...
  return whatsappRequest('/config', config);
}

async function whatsappSelectGroup(groupId: string) {
  return whatsappRequest('/group', { groupId });
}

async function whatsappRefreshGroups() {
  return whatsappRequest('/groups/refresh');
}

async function whatsappLogout() {
  return whatsappRequest('/logout');
}
...
async function calculateBudgetsFromTransactions(
  addedIds: string[],
): Promise<void> {
  ...
  // Get all categories to check is_income flag
  type CategoryRow = {
    id: string;
    name: string;
    is_income: boolean;
  ...
  // Fetch the full transaction objects from the database
  ...
  // Include both expenses (amount < 0) and income (amount > 0)
  ...
  for (const tx of transactions) {
    ...
    // Skip uncategorized and Transfer categories
    ...
    budgetsByMonth[month][categoryId] =
      (budgetsByMonth[month][categoryId] || 0) + Math.abs(tx.amount);
  }

  // Apply budgets for each month/category
    ...
    for (const [categoryId, amount] of Object.entries(budgets)) {
      try {
        await setBudget({
          category: categoryId,
          month,
          amount: Math.round(amount), // Already in cents from transaction
        });
      } catch (e) {
        logger.log(`Warning: failed to set budget for ${month}/${categoryId}: ${e instanceof Error ? e.message : String(e)}`);
      }
    }
  }
}
...
async function importTransactions({
  ...
  try {
    ...
    // Auto-calculate budgets based on imported transactions (only if not preview)
    if (!isPreview && reconciled.added.length > 0) {
      await calculateBudgetsFromTransactions(reconciled.added);
    }
...
app.method('whatsapp-status', whatsappStatus);
app.method('whatsapp-connect', whatsappConnect);
app.method('whatsapp-save-config', whatsappSaveConfig);
app.method('whatsapp-select-group', whatsappSelectGroup);
app.method('whatsapp-refresh-groups', whatsappRefreshGroups);
app.method('whatsapp-logout', whatsappLogout);
